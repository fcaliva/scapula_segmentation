...
class VNet_withAux(object):
    ...
    def network_fn(self, x, keep_prob):
        ...
        with tf.variable_scope(f'{name}/auxClassification_level'):
            with tf.variable_scope('global_average_pooling'):
                if self.data_format == 'channels_first':
                    ...
                    if spatial_rank == 2:
                        ...
                        axis = [1,2,3]
                aux = tf.reduce_mean(x,axis=axis)
            with tf.variable_scope('dense_layer'):
                aux = tf.layers.Dense(units = 16)(aux)
            with tf.variable_scope('classification_layer'):
                aux_logits = tf.layers.Dense(units = 1)(aux)
